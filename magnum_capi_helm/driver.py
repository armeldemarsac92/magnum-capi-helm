...
class Driver(driver.Driver):
    ...
    def _get_etcd_config(self, cluster):
        ...
        etcd_size = self._get_label_int(
            ...
            self._get_label_int(
                cluster,
                "etcd_volume_size",
                CONF.capi_helm_cluster_labels.etcd_blockdevice_size,
            ),
        )
        if etcd_size > 0:
            ...
            etcd_bd_type = self._label(
                ...
                CONF.capi_helm_cluster_labels.etcd_blockdevice_type,
            )
            if etcd_bd_type == "local":
                ...
                etcd_volume_type = self._label(
                    ...
                    self._label(
                        cluster,
                        "etcd_volume_type",
                        CONF.capi_helm_cluster_labels.etcd_blockdevice_volume_type,
                    ),
                )
                if etcd_volume_type:
                ...
                etcd_volume_az = self._label(
                    ...
                    CONF.capi_helm_cluster_labels.etcd_blockdevice_volume_az,
                )
                if etcd_volume_az:
    ...
    def _update_helm_release(self, context, cluster, nodegroups=None):
        ...
        values = {
            ...
            "apiServer": {
                "associateFloatingIP": self._get_label_bool(
                    ...
                    CONF.capi_helm_cluster_labels.master_lb_floating_ip_enabled,
                ),
            ...
            "clusterNetworking": {
                ...
                "internalNetwork": {
                    ...
                    "nodeCidr": self._label(
                        cluster,
                        "fixed_subnet_cidr",
                        CONF.capi_helm_cluster_labels.fixed_subnet_cidr,
                    ),
                },
            },
            "osDistro": os_distro,
                    ...
                    "cloudConfig": {
                        "LoadBalancer": {
                            ...
                            "create-monitor": self._get_label_bool(
                                ...
                                CONF.capi_helm_cluster_labels.octavia_lb_healthcheck,
                            ),
                        }
                    },
                },
        ...
        boot_volume_type = self._label(
            cluster,
            "boot_volume_type",
            CONF.capi_helm_cluster_labels.boot_volume_type
            or CONF.cinder.default_boot_volume_type,
        )
        if boot_volume_type:
            disk_type_details = {
                "controlPlane": {
        ...
        conf_boot_size = CONF.capi_helm_cluster_labels.boot_volume_size
        ...
        extra_network_name = self._label(
            cluster,
            "extra_network_name",
            CONF.capi_helm_cluster_labels.extra_network_name,
        )
