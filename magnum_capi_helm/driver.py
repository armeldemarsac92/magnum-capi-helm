...
class Driver(driver.Driver):
    ...
    def _get_octavia_provider(self, cluster):
        return self._label(cluster, "octavia_provider", "amphora").lower()

    def _get_octavia_lb_algorithm(self, cluster):
        provider = self._get_octavia_provider(cluster)
        if provider == "ovn":
            # NOTE(scott): OVN only supports one algorithm
            return "SOURCE_IP_PORT"
        return self._label(cluster, "octavia_lb_algorithm", "ROUND_ROBIN")
