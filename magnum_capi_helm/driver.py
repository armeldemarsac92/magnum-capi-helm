...
class Driver(driver.Driver):
    ...
    def _get_label_array(self, cluster, label, default):
        cluster_label = self._label(cluster, label, "")
        if not cluster_label:
            return default
        try:
            return strutils.split_by_commas(cluster_label)
        except ValueError:
            return default
    ...
    def _update_helm_release(self, context, cluster, nodegroups=None):
        ...
        # Set control plane availability_zone details if they are provided
        control_plane_availability_zone = self._label(
            cluster, "control_plane_availability_zone", ""
        )
        if control_plane_availability_zone:
            az_details = {
                "controlPlane": {
                    "omitFailureDomain": False,
                    "failureDomains": [control_plane_availability_zone],
                },
            }
            values = helm.mergeconcat(values, az_details)

        # Set worker availability_zone details if they are provided
