...
class ClusterAPIDriverTest(base.DbTestCase):
    ...
    def test_validate_auto_scale_without_node_count(self):
    ...
    def test_validate_auto_scale_max_lt_min(self):
        self.cluster_obj.labels = dict(
            auto_scaling_enabled="true", min_node_count=3, max_node_count=2
        )
        self.assertRaises(
            exception.MagnumException,
            self.driver._get_autoscale,
    ...
    def test_create_cluster_auto_scale_enabled(
        ...
        helm_install_values = mock_install.call_args[0][3]
        self.assertEqual(
            helm_install_values["nodeGroupDefaults"]["autoscale"],
            auto_scale_labels["auto_scaling_enabled"],
        )
        self.assertEqual(
            helm_install_values["nodeGroupDefaults"]["machineCountMin"],
            auto_scale_labels["min_node_count"],
        )
        self.assertEqual(
            helm_install_values["nodeGroupDefaults"]["machineCountMax"],
    ...
    def test_create_cluster_auto_scale_disabled(
        ...
        self.assertNotIn(
            "autoscale",
            helm_install_values["nodeGroupDefaults"],
        )
        self.assertNotIn(
            "machineCountMin",
            helm_install_values["nodeGroupDefaults"],
